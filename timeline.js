...
links.Timeline = function(container) {
    ...
    this.options = {
        ...
        'resizable': false, //make the chart horizonal resizable
...
links.Timeline.prototype.draw = function(data, options, isJSON) {
    ...
    if (this.options.resizable)
    {
        ...
        moveBar.className = 'frame-move-width';
        this.dom.frame.appendChild(moveBar);

        this.makeResizable(this.dom.frame,moveBar);
    }
...
links.Timeline.ItemLine.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        var left = timeline.timeToScreen(this.start),
            ...
            axisTop = timeline.size.contentHeight,
            axisHeight = timeline.size.axis.height


        dom.style.left = (left - this.lineWidth / 2) + "px";
        dom.style.top = (timeline.size.axis.height+1)+"px";
...
links.Timeline.ItemRange.prototype.updatePosition = function (timeline) {
    ...
    if (dom) {
        ...
        var _w = dom.firstChild.clientWidth;
        var _W = dom.clientWidth;

        var d1 = timeline.screenToTime(left);
        var d2 = timeline.screenToTime(left+ Math.max(right - left, 1));

        var obj = {};
        timeline.step.current = d1;
        obj.start = timeline.step.getLabelMinor(timeline.options);
        timeline.step.current = d2;
        obj.end = timeline.step.getLabelMinor(timeline.options);

        dom.firstChild.firstChild.innerHTML = obj.start;
        dom.firstChild.firstChild.nextSibling.nextSibling.innerHTML = obj.end;
...
links.Timeline.prototype.trigger = function (event) {
    ...
    links.events.trigger(this,'eventtriggered',{ eventName: event});
...
links.Timeline.parseJSONDate = function (date) {
    ...
    var ms = date.match(/Date\(\s*(\d+)\s*\,\s*(\d+)\s*\,\s*(\d+)\s*\,\s*(\d+)\s*\,\s*(\d+)\s*\,\s*(\d+)\s*\,?\s*(\d*)/i);
    if (ms) {
        var t = new Date( parseInt(ms[1]),
            parseInt(ms[2])-1,
            parseInt(ms[3]),
            parseInt(ms[4]),
            parseInt(ms[5]),
            parseInt(ms[6]),
            parseInt(ms[7] ? ms[7] : '0'));
        return t;
    }
...
/**
 * parse a JSON object
 * @param {String} json_string    JSON string to be parsed. 
 * @return {Object} object
 */
links.Timeline.parseJSON = function (s) {
...
links.Timeline.getStartEndTime = function(d1,d2)
{
    var Months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    var t1 = d1.getTime();
    var t2 = d2.getTime();
    var g = Math.abs(t1-t2);
    if (g < 1000) //less than 1000ms
    {
        return {start: t1 % 1000, end: t2 %1000};
    }
    t1 = Math.floor(t1 / 1000);
    t2 = Math.floor(t2 / 1000);
    var g = Math.abs(t1-t2);
    if (g < 60) return {start: d1.getSeconds(), end: d2.getSeconds()};
    if (g < 3600) return {start: d1.getMinutes()+':'+toFixed(d1.getSeconds()), end:d2.getMinutes()+':'+toFixed(d2.getSeconds())};

    if (g < 86400) return {start: d1.getHours()+':'+toFixed(d1.getMinutes()), end:d2.getHours()+':'+toFixed(d2.getMinutes())};
    if (g < 86400*30) return {start: d1.getDate(), end:d2.getDate()};

    return {start: Months[d1.getMonth()]+' '+d1.getDate(), end: Months[d2.getMonth()]+' '+d2.getDate()};
...
links.Timeline.prototype.makeResizable = function(dom,handler)
{
    var self = this;
    links.Timeline.addEventListener(handler,'mousedown',function(event)
    {
        links.Timeline.preventDefault(event);
        links.Timeline.stopPropagation(event);
        var mouseX = links.Timeline.getPageX(event);
        var startWidth = dom.clientWidth;


        var mousemove = function(event)
        {
            var x = links.Timeline.getPageX(event);
            var gap = x - mouseX;
            var newWidth = startWidth + gap;
